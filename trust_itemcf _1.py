...
class ItemBasedCF():
    ...
    def trust(self,item,other,lambd=0.5):
        ...
        if total==0.0:
            return 0.0
        if(self.item_rating_count[other]>=self.alpha*self.item_ave):
            return count*total/self.item_rating_count[item]

        return count*total*self.item_rating_count[other]\
               /(self.item_rating_count[item]*self.alpha*self.item_ave)


    def get_trust_matrix(self,alpha=0.5):
        result={}
    ...
    def get_indirect_sim(self,item,other):
        total_sim = 0.0
        total_trust = 0.0


        for i in range(self.trust_neighbour):
    ...
    def get_sim_matrix(self,alpha=0.4):
        result={}
        for item in self.trans_prefs:
            result.setdefault(item, {})
            if self.item_rating_count[item]>self.alpha*self.item_ave:
