"user_based collaborative filtering"
import sys, random, math
from operator import itemgetter
from math import sqrt



class UserBasedCF():
    ''' TopN recommendation - UserBasedCF '''
    def __init__(self):
        self.trainset = {}
        self.testset = {}

        self.n_sim_user = 500

        self.sim_mat = {}
        #self.movie_popular = {}
        #self.movie_count = 0

        print >> sys.stderr, 'Similar user number = %d' % self.n_sim_user



    '''load the trainset'''
    def load_train_data(self, filename):
        fp = open(filename, 'rb')
        line = fp.readline()
        # line=fp.readline()
        trainset_len = 0
        while line != '':
            line.strip('\r\n')
            user, item, rating, time = line.split(',')
            self.trainset.setdefault(user, {})
            self.trainset[user][item] = float(rating)
            trainset_len += 1
            line = fp.readline()

        fp.close()
        print >> sys.stderr, 'train set = %s' % trainset_len



    '''load the testset'''
    def load_test_data(self, filename):
        fp = open(filename, 'rb')
        # line = fp.readline()
        line = fp.readline()
        testset_len = 0
        while line != '':
            line.strip('\r\n')
            user, item, rating, time = line.split(',')
            self.testset.setdefault(user, {})
            self.testset[user][item] = float(rating)
            testset_len += 1
            line = fp.readline()

        fp.close()
        print >> sys.stderr, 'test set = %s' % testset_len


    '''calculate the similarity of the two users'''
    def cos_sim(self, user1, user2):
        si = {}
        for item in self.trainset[user1]:
            if item in self.trainset[user2]: si[item] = 1

        if len(si) == 0: return 0.0

        sum1sq = sum([pow(self.trainset[user1][item], 2) for item in si])
        sum2sq = sum([pow(self.trainset[user2][item], 2) for item in si])
        sum1 = sqrt(sum1sq)
        sum2 = sqrt(sum2sq)

        sum_of_item = sum([self.trainset[user1][item] * self.trainset[user2][item] for item in si])


        return sum_of_item / float(sum1 * sum2)



    '''get the simlarity matrix of user'''
    def get_sim_matrix(self):
        sim_matrix = {}

        for user in self.trainset:
            sim_matrix.setdefault(user, {})
            for other in self.trainset:
                if other == user: continue
                if self.cos_sim(user,other)!=0.0:
                    sim_matrix[user][other] = self.cos_sim(user, other)

            # sort the similarity
            self.sim_mat[user] = sorted(sim_matrix[user].iteritems(), key=lambda b: b[1], reverse=True)

        print "have finished calculate the similarity matrix"


    '''predict the ratings'''
    def predict_rating(self, user, item):
        r = 0.0
        totalsim = 0.0

        for i in range(self.n_sim_user):
            other = self.sim_mat[user][i][0]
            if item in self.trainset[other].keys():
                r = r + self.trainset[other][item] * self.sim_mat[user][i][1]
                totalsim=totalsim+self.sim_mat[user][i][1]

        '''cannot predict the rating if totalsim equals 0.0, return 0.0 '''
        if totalsim == 0.0: return 0.0

        return r / totalsim


    '''evaluate the performance with the RMSE'''
    def rmse(self):
        n = 0.0
        sum_r = 0.0
        total = 0.0

        for user in self.testset.keys():
            if user not in self.trainset:continue
            for item in self.testset[user].keys():
                total = total + 1

                if user not in self.sim_mat.keys():
                    continue

                rating = self.predict_rating(user, item)
                if rating != 0.0:
                    n = n + 1
                    sum_r = sum_r+pow(self.testset[user][item] - rating, 2)
        print "the coverage is ", n /total
        print "the rmse is", sqrt(sum_r / n)



if __name__ == '__main__':
    trainfile = r'test1/train_rating.txt'
    testfile  = r'test1/train_rating.txt'
    usercf = UserBasedCF()
    usercf.load_train_data(trainfile)
    usercf.load_test_data(testfile)
    usercf.get_sim_matrix()
    usercf.rmse()


